    ...
    <Dialog open onOpenChange={(open) => { if (!open) close() }} eyebrow="Report provenance" title="Register signing key">
        {!result ? (
          ...
          <div className="mt-8">
            ...
            <div className="mt-5 rounded-xl border border-stone-800 bg-[#eee8dd] p-4"><p className="detail-label">Ansible variables</p><div className="mt-3 flex items-start gap-3"><code className="min-w-0 flex-1 whitespace-pre-wrap font-mono text-xs leading-6 text-stone-300">{configuration}</code><Button variant="ghost" size="icon" className="shrink-0" onClick={() => void copyConfiguration()} aria-label="Copy signing key configuration">{copied ? <Check size={16} /> : <Copy size={16} />}</Button></div></div>
