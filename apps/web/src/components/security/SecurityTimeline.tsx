...
export function SecurityTimeline({ events, empty = 'No recent activity.' }: { events: TimelineEvent[]; empty?: string }) {
  if (!events.length) return <p className="px-5 py-10 text-center text-xs text-stone-600">{empty}</p>
  return <ol className="security-timeline">{events.map((event) => {
    ...
    const content = <>
      ...
      <div className="min-w-0 flex-1"><p className="truncate text-xs font-medium text-stone-800">{event.title}</p><p className="mt-1 truncate text-[11px] text-stone-600">{event.detail}</p></div>
      <time className="shrink-0 font-mono text-[9px] text-stone-500" dateTime={event.timestamp ?? undefined}>{formatCompactDateTime(event.timestamp)}</time>
    </>
