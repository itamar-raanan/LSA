    ...
    <Dialog open onOpenChange={(open) => { if (!open) close() }} eyebrow="Scanner credential" title="Issue ingestion token">
        {!result ? (
          <form className="space-y-5" onSubmit={submit}>
            ...
            {!hostId && <p className="rounded-xl border border-amber-900/40 bg-amber-950/15 px-4 py-3 text-xs leading-5 text-amber-800">This credential will be tenant-wide. Prefer a host scope unless one controller intentionally reports for several systems.</p>}
          ...
          <div className="mt-8">
            ...
            <div className="mt-5 rounded-xl border border-stone-200 bg-[#eee8dd] p-4"><p className="detail-label">Ingestion token</p><div className="mt-3 flex items-center gap-3"><code className="min-w-0 flex-1 overflow-x-auto font-mono text-xs leading-6 text-stone-700">{result.token}</code><Button variant="ghost" size="icon" className="shrink-0" onClick={() => void copyToken()} aria-label="Copy ingestion token">{copied ? <Check size={16} /> : <Copy size={16} />}</Button></div></div>
