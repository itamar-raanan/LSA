...
export function AgentDownloadPanel({ packages, enrollmentToken, close }: AgentDownloadPanelProps) {
  ...
  const installCommand = useMemo(
    () => primaryPackage
      ? `tar -xzf ${primaryPackage.filename}\ncd lsa-agent-${primaryPackage.version}\nsudo ./install.sh --platform-url ${shellQuote(platformUrl)} --token ${shellQuote(tokenValue)}`
      : 'No agent package is available.',
    [platformUrl, primaryPackage, tokenValue],
  )
  ...
  return <section className="panel mb-7 overflow-hidden" aria-labelledby="agent-download-title">
    ...
    {!packages.length ? <div className="px-6 py-10 text-sm text-stone-500">No agent release is available.</div> : packages.map(agentPackage => <div key={agentPackage.id} className="grid gap-6 px-6 py-6 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
      <div className="flex min-w-0 items-start gap-4">
        ...
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2"><h3 className="text-sm font-medium text-stone-200">LSA Agent {agentPackage.version}</h3><span className="settings-state">Universal Linux</span></div>
      ...
      <button className="button-primary min-w-40" disabled={downloading === agentPackage.id} onClick={() => void download(agentPackage)}><DownloadSimple size={16} /> {downloading === agentPackage.id ? 'Downloading…' : 'Download package'}</button>
    </div>)}

    <div className="border-t border-stone-800 bg-[#101411] px-6 py-6">
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-2 text-xs font-medium text-stone-300"><TerminalWindow size={16} className="text-emerald-400" /> Install and enroll</div>
      ...
      <p className="mt-3 text-[11px] leading-5 text-stone-600">Requires Python 3.11+, systemd, and network access to install constrained Python dependencies. For a private CA, copy its certificate to the host and add <code className="text-stone-500">--ca-bundle /path/to/ca.pem</code>.</p>
