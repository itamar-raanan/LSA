...
export function HostsPage() {
  ...
  const columns: SecurityColumn<Host>[] = [
    ...
    { id: 'findings', header: 'Open exposure', priority: 'detail', sortValue: (host) => host.finding_counts.critical * 1000 + host.finding_counts.high, exportValue: (host) => `${host.finding_counts.critical} critical; ${host.finding_counts.high} high`, cell: (host) => <span className="table-primary"><span className={host.finding_counts.critical ? 'text-rose-400' : 'text-slate-300'}>{host.finding_counts.critical} critical</span><small>{host.finding_counts.high} high · {host.finding_counts.medium} medium</small></span> },
    { id: 'last_seen', header: 'Last seen', priority: 'detail', sortValue: (host) => host.last_scan_at ?? '', exportValue: (host) => host.last_scan_at, cell: (host) => <span className="font-mono text-[10px] text-slate-400">{host.last_scan_at ? new Date(host.last_scan_at).toLocaleString() : 'Never'}</span> },
  ]
