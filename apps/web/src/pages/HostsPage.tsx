...
export function HostsPage() {
  ...
  const columns: SecurityColumn<Host>[] = [
    ...
    { id: 'risk', header: 'Security Score', priority: 'secondary', sortValue: (host) => host.security_score ?? -1, exportValue: (host) => host.security_score, cell: (host) => <RiskScore value={host.security_score ?? 0} label="posture" size="sm" /> },
    { id: 'findings', header: 'Open Exposure', priority: 'secondary', sortValue: (host) => host.finding_counts.critical * 1000 + host.finding_counts.high, exportValue: (host) => `${host.finding_counts.critical} critical; ${host.finding_counts.high} high`, cell: (host) => <span className="table-primary"><span className={host.finding_counts.critical ? 'text-rose-700' : 'text-stone-700'}>{host.finding_counts.critical} Critical</span><small>{host.finding_counts.high} High · {host.finding_counts.medium} Medium</small></span> },
