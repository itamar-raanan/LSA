      ...
      <section className="grid gap-4 xl:grid-cols-[1.15fr_0.65fr]">
        <div className="panel p-6 md:p-8">
          ...
          <button type="button" className={`upload-zone mt-7 ${file ? 'border-[#b8c5ba] bg-[#edf1eb]' : ''}`} onClick={() => inputRef.current?.click()} onDragOver={(event) => event.preventDefault()} onDrop={drop}>
            ...
            <span className="mt-4 text-sm font-medium text-stone-800">{file ? file.name : 'Choose or drop a report bundle'}</span>
          ...
          {message && <div className={`mt-5 flex items-start gap-3 rounded-xl border px-4 py-3 text-xs leading-5 ${status === 'success' ? 'border-[#b8c5ba] bg-[#edf1eb] text-[#4f6f5c]' : 'border-rose-900/50 bg-rose-950/20 text-rose-700'}`}>{status === 'success' ? <CheckCircle size={17} className="mt-0.5 shrink-0" /> : <WarningCircle size={17} className="mt-0.5 shrink-0" />}{message}</div>}
        ...
        <aside className="panel p-6 md:p-8">
          ...
          <div className="mt-8 border-t border-stone-200 pt-6"><div className="flex items-center gap-2 text-xs font-medium text-stone-700"><ShieldCheck size={17} className="text-[#4f6f5c]" />No server access required</div><p className="mt-3 text-xs leading-5 text-stone-600">The platform receives evidence only. It cannot open an SSH session or execute remediation on a reporting host.</p></div>
        </aside>
      </section>
    </div>
  )
