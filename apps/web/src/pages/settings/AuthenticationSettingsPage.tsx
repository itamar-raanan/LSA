  ...
  return <div className="page-reveal">
    ...
    <section className="panel overflow-hidden">
      <div className="grid gap-4 px-6 py-6 sm:grid-cols-[44px_1fr_auto] sm:items-center md:px-7">
        <span className="grid size-11 place-items-center rounded-xl border border-amber-900/50 bg-amber-950/20 text-amber-800"><LockKey size={21} weight="duotone" /></span>
        <div><p className="text-sm font-medium text-stone-800">Emergency local administrator</p><p className="mt-1 text-xs leading-5 text-stone-600">Break-glass access only. Regular users are JIT-provisioned from OIDC or RADIUS.</p></div>
    ...
    {actionError && <p className="mt-4 rounded-xl border border-rose-900/50 bg-rose-950/20 px-4 py-3 text-xs text-rose-700">{actionError}</p>}
    <div className="mt-5">
      {loading ? <LoadingState variant="settings" /> : error ? <ErrorState message={error} retry={reload} /> : !providers?.length ? <EmptyState title="No identity providers" detail="Add Entra ID, Okta, Google Workspace, ADFS, generic OpenID Connect, or RADIUS." action={<button className="button-primary" onClick={() => setAdding(true)}><Plus size={15} />Add Provider</button>} /> : <div className="divide-y divide-stone-200 overflow-hidden rounded-[22px] border border-stone-200 bg-[#f7f3eb]">{providers.map((provider) => <section key={provider.id} className="grid gap-5 px-6 py-6 md:grid-cols-[44px_1fr_auto] md:items-center md:px-7">
        <span className="grid size-11 place-items-center rounded-xl border border-stone-200 bg-[#f7f3eb] text-stone-500">{provider.provider_type === 'radius' ? <Network size={21} /> : <Buildings size={21} />}</span>
        <div><p className="text-sm font-medium text-stone-800">{provider.name}</p><p className="mt-2 text-xs text-stone-600">{providerLabels[provider.provider_type]} · {provider.issuer_url ?? String(provider.config.host ?? '')}</p><p className="mt-2 font-mono text-[9px] capitalize tracking-wider text-stone-700">Secret {provider.secret_configured ? 'configured' : 'missing'} · default role auditor</p></div>
    ...
    <div className="mt-5 flex items-start gap-3 rounded-[18px] border border-amber-900/40 bg-amber-950/10 px-5 py-4 text-xs leading-5 text-amber-800/70"><WarningCircle size={18} className="mt-0.5 shrink-0" />Provider secrets are encrypted. OIDC uses discovery, authorization code with PKCE, state, nonce, issuer, audience, expiry, and signing-key validation. RADIUS should remain on a protected internal network.</div>
    <Dialog
      open={deleting !== null}
    ...
    >
      <div className="rounded-lg border border-rose-900/40 bg-rose-950/10 px-4 py-3 text-xs leading-5 text-rose-700">
