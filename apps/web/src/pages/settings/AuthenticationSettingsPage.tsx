...
export function AuthenticationSettingsPage() {
  ...
  return <div className="page-reveal">
    ...
    <div className="mt-5">
      {loading ? <LoadingState /> : error ? <ErrorState message={error} retry={reload} /> : !providers?.length ? <EmptyState title="No identity providers" detail="Add Entra ID, Okta, Google Workspace, ADFS, generic OpenID Connect, or RADIUS." /> : <div className="divide-y divide-stone-800 overflow-hidden rounded-[22px] border border-stone-800 bg-[#151916]">{providers.map((provider) => <section key={provider.id} className="grid gap-5 px-6 py-6 md:grid-cols-[44px_1fr_auto] md:items-center md:px-7">
        <span className="grid size-11 place-items-center rounded-xl border border-stone-800 bg-[#111512] text-stone-500">{provider.provider_type === 'radius' ? <Network size={21} /> : <Buildings size={21} />}</span>
        <div><p className="text-sm font-medium text-stone-200">{provider.name}</p><p className="mt-2 text-xs text-stone-600">{providerLabels[provider.provider_type]} · {provider.issuer_url ?? String(provider.config.host ?? '')}</p><p className="mt-2 font-mono text-[9px] uppercase tracking-wider text-stone-700">Secret {provider.secret_configured ? 'configured' : 'missing'} · default role auditor</p></div>
