...
export function CertificatesSettingsPage() {
  ...
  return <div className="page-reveal">
    ...
    {loading ? <LoadingState variant="settings" /> : error ? <ErrorState message={error} retry={reload} /> : <section className="panel overflow-hidden">
      <div className="grid gap-6 px-6 py-7 md:grid-cols-[1fr_1.15fr] md:px-7">
        <div><span className="grid size-11 place-items-center rounded-xl border border-stone-200 bg-[#f7f3eb] text-[#4f6f5c]"><Certificate size={22} weight="duotone" /></span><h2 className="mt-5 text-lg font-medium tracking-tight text-stone-800">{certificate ? 'Active certificate' : 'Certificate upload'}</h2>
          {certificate ? <dl className="mt-4 space-y-3 text-xs text-stone-500"><div><dt className="detail-label">Subject</dt><dd className="mt-1 break-all">{certificate.subject}</dd></div><div><dt className="detail-label">Issuer</dt><dd className="mt-1 break-all">{certificate.issuer}</dd></div><div><dt className="detail-label">DNS names</dt><dd className="mt-1">{certificate.hostnames.join(', ') || 'None'}</dd></div><div><dt className="detail-label">Valid until</dt><dd className="mt-1">{formatDateTime(certificate.not_valid_after)}</dd></div><div><dt className="detail-label">SHA-256</dt><dd className="mt-1 break-all font-mono text-[10px]">{certificate.fingerprint}</dd></div></dl> : <p className="mt-2 text-xs leading-5 text-stone-600">A short-lived self-signed localhost certificate is generated at first boot so the platform never exposes plaintext HTTP.</p>}</div>
        <form className="rounded-[18px] border border-dashed border-stone-700 bg-[#f7f3eb] p-6" onSubmit={upload}>
          <UploadSimple size={25} className="text-stone-600" /><p className="mt-4 text-sm text-stone-700">Rotate certificate</p><p className="mt-2 text-xs leading-5 text-stone-600">Upload the server certificate or certificate chain as a .crt/.pem file and its matching unencrypted .key/.pem private key. PEM and DER encoding are accepted.</p>
      ...
      <div className="grid divide-y divide-stone-200 border-t border-stone-200 bg-[#f7f3eb] sm:grid-cols-3 sm:divide-x sm:divide-y-0"><div className="px-5 py-4"><p className="detail-label">Formats</p><p className="mt-2 text-xs text-stone-500">CRT + KEY · PEM or DER</p></div><div className="px-5 py-4"><p className="detail-label">Validation</p><p className="mt-2 text-xs text-stone-500">Validity and key match</p></div><div className="px-5 py-4"><p className="detail-label">Rotation</p><p className="mt-2 text-xs text-stone-500">Atomic gateway reload</p></div></div>
    </section>}
    <div className="mt-5 flex items-start gap-3 rounded-[18px] border border-amber-900/40 bg-amber-950/10 px-5 py-4 text-xs leading-5 text-amber-800/70"><WarningCircle size={18} className="mt-0.5 shrink-0" /><span><strong className="font-medium text-amber-800">Private keys never leave the backend response.</strong> They are encrypted in PostgreSQL, materialized with restricted permissions in the internal TLS volume, and omitted from audit data.</span></div>
