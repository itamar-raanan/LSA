...
export function UsersSettingsPage() {
  ...
  return <div className="page-reveal">
    ...
    {adding && <section className="panel mb-6 overflow-hidden">
      <div className="flex items-start justify-between gap-4 border-b border-stone-200 px-6 py-5"><div><p className="section-label">External identity</p><h2 className="mt-2 text-base font-semibold text-stone-800">Pre-provision user</h2><p className="mt-2 text-xs leading-5 text-stone-600">LSA stores the identity link and role; authentication remains with your selected provider.</p></div><button className="icon-button" aria-label="Close add user form" onClick={() => setAdding(false)}><X size={16} /></button></div>
      <form className="grid gap-4 px-6 py-6 md:grid-cols-2" onSubmit={(event) => void createUser(event)}>
        ...
        {formError && <p className="text-xs text-rose-700 md:col-span-2">{formError}</p>}
      </form>
    </section>}
    {!providers?.length && !loading && <div className="mb-6 rounded-xl border border-amber-900/40 bg-amber-950/10 px-4 py-3 text-xs text-amber-800">Configure an identity provider before pre-provisioning users.</div>}
    {loading ? <LoadingState variant="settings" /> : error ? <ErrorState message={error} retry={reload} /> : !users?.length ? <EmptyState title="No users" detail="Enable an identity provider and complete the first organization sign-in." action={providers?.length ? <button className="button-primary" onClick={() => setAdding(true)}><Plus size={15} />Add First User</button> : <Link className="button-primary" to="/settings/authentication">Configure Authentication</Link>} /> : <section className="panel overflow-hidden">
      ...
      <div className="flex items-start gap-3 border-t border-stone-200 bg-[#f7f3eb] px-6 py-4 text-xs leading-5 text-stone-600"><UsersThree size={17} className="mt-0.5 shrink-0" />Disabling a user immediately revokes every active browser session. User identities remain linked to their provider subject.</div>
    </section>}
