...
function validationSummary(job: RemediationValidationJob) {
  if (!job.receipt) return null
  const actionSummary = `${job.receipt.action_results.filter(result => result.status === 'ready').length} Of ${job.receipt.action_results.length} Actions Ready`
  const recovery = job.receipt.recovery_plan
  if (!recovery) return `${actionSummary} · Recovery Planning Not Reported · No Changes Applied`
  const readyCheckpoints = recovery.entries.filter(entry => entry.status === 'ready').length
  const recoverySummary = recovery.status === 'ready'
    ? `${readyCheckpoints} Recovery Checkpoint${readyCheckpoints === 1 ? '' : 's'} Ready`
    : `Recovery Plan Blocked · ${readyCheckpoints} Of ${recovery.entries.length} Checkpoints Ready`
  return `${actionSummary} · ${recoverySummary} · No Changes Applied`
}

function ChangeSetDossier({ changeSet, admin, currentUserId, validations, checkpoints, verifications, validationLoading, validationError, validationBusyAgent, checkpointBusyValidation, verificationBusyCheckpoint, authorize, cancel, queueValidation, queueCheckpoint, queueVerification }: { changeSet: RemediationChangeSet; admin: boolean; currentUserId?: string; validations: RemediationValidationJob[]; checkpoints: RemediationCheckpointJob[]; verifications: RemediationRecoveryVerificationJob[]; validationLoading: boolean; validationError: string | null; validationBusyAgent: string | null; checkpointBusyValidation: string | null; verificationBusyCheckpoint: string | null; authorize: () => void; cancel: () => void; queueValidation: (agentId: string) => void; queueCheckpoint: (validationId: string) => void; queueVerification: (checkpointId: string) => void }) {
  ...
  return <aside className="change-set-dossier" aria-label={`Change Set ${changeSet.id} Dossier`}>
    ...
    <section className="change-set-section"><div className="change-set-section-heading"><h3>Canary Rollout And Read-Only Preflight</h3><span>Batch {changeSet.batch_size} · Every {changeSet.batch_interval_minutes} Minutes</span></div><div className="change-set-targets">{changeSet.targets.map(target => {
      ...
      const verification = checkpoint ? verifications.find(item => item.checkpoint_job_id === checkpoint.id) : undefined
      const verificationActive = verification?.status === 'queued' || verification?.status === 'delivered'
      return <div key={target.host_id}><span className={`change-set-phase change-set-phase-${target.rollout_phase}`}>{target.rollout_phase}</span><div><strong>{target.hostname}</strong><p>{target.group_name} · {target.policy_name} V{target.policy_version}</p>{job?.receipt && <small>{validationSummary(job)}</small>}{checkpoint && <small>{checkpoint.status === 'ready' ? `${checkpoint.receipt?.checkpoint_results.length ?? 0} Encrypted Checkpoint${checkpoint.receipt?.checkpoint_results.length === 1 ? '' : 's'} Prepared On The Agent` : checkpoint.status === 'blocked' ? `Checkpoint Blocked · ${checkpoint.error ?? 'Run It Again After Resolving The Reported Condition'}` : 'Encrypted Checkpoint Is Being Prepared'}</small>}{verification && <small>{verification.status === 'ready' ? `${verification.receipt?.verification_results.length ?? 0} Recovery Checkpoint${verification.receipt?.verification_results.length === 1 ? '' : 's'} Authenticated And Decrypted Safely` : verification.status === 'blocked' ? `Recovery Verification Blocked · ${verification.error ?? 'Prepare A New Checkpoint After Resolving The Reported Condition'}` : 'Recovery Readiness Is Being Verified'}</small>}{job?.error && <small>{job.error}</small>}</div><div className="change-set-target-validation">{job ? <ValidationStatus job={job} /> : <span>{validationLoading ? 'Loading Preflight' : target.capability_attested ? 'Not Evaluated' : 'Capability Missing'}</span>}{admin && changeSet.status === 'authorized' && !active && job?.status !== 'ready' && <Button variant="ghost" disabled={validationBusyAgent === target.agent_id} onClick={() => queueValidation(target.agent_id)}>{validationBusyAgent === target.agent_id ? 'Queueing' : job ? 'Run Again' : 'Run Read-Only Preflight'}</Button>}{admin && job?.status === 'ready' && !checkpointActive && checkpoint?.status !== 'ready' && <Button variant="ghost" disabled={checkpointBusyValidation === job.id} onClick={() => queueCheckpoint(job.id)}>{checkpointBusyValidation === job.id ? 'Queueing Checkpoint' : checkpoint ? 'Retry Encrypted Checkpoint' : 'Prepare Encrypted Checkpoint'}</Button>}{admin && checkpoint?.status === 'ready' && !verificationActive && verification?.status !== 'ready' && <Button variant="ghost" disabled={verificationBusyCheckpoint === checkpoint.id} onClick={() => queueVerification(checkpoint.id)}>{verificationBusyCheckpoint === checkpoint.id ? 'Queueing Verification' : verification ? 'Verify Recovery Again' : 'Verify Recovery Readiness'}</Button>}</div></div>
    })}</div>{validationError && <p className="remediation-decision-error" role="alert">{validationError}</p>}<div className="change-set-validation-note"><ShieldX size={15} /><p><strong>Recovery Verification Cannot Change Host Configuration.</strong> The agent authenticates and decrypts local checkpoint material in memory, compares its original digest, and returns signed evidence only. Plaintext, restore, and remediation execution remain unavailable.</p></div><div className="change-set-window"><CalendarClock size={16} /><span><strong>{formatDateTime(changeSet.maintenance_window_start)}</strong><small>Through {formatDateTime(changeSet.maintenance_window_end)}</small></span></div></section>
...
export function ChangeSetsPage() {
  ...
  const verifications = useApi(() => selected ? api.remediationRecoveryVerificationJobs(selected.id) : Promise.resolve([]), [selected?.id])
  ...
  const [verificationBusyCheckpoint, setVerificationBusyCheckpoint] = useState<string | null>(null)
  const hasActiveValidations = (validations.data?.some(job => job.status === 'queued' || job.status === 'delivered') ?? false) || (checkpoints.data?.some(job => job.status === 'queued' || job.status === 'delivered') ?? false) || (verifications.data?.some(job => job.status === 'queued' || job.status === 'delivered') ?? false)
  const refreshValidations = validations.refresh
  const refreshCheckpoints = checkpoints.refresh
  const refreshVerifications = verifications.refresh

  useEffect(() => {
    if (!hasActiveValidations) return
    const interval = window.setInterval(() => { void refreshValidations(); void refreshCheckpoints(); void refreshVerifications() }, 10_000)
    return () => window.clearInterval(interval)
  }, [hasActiveValidations, refreshCheckpoints, refreshValidations, refreshVerifications])
  ...
  async function queueVerification(checkpointId: string) {
    if (!selected) return
    setVerificationBusyCheckpoint(checkpointId); setMutationError('')
    try { await api.queueRemediationRecoveryVerification(selected.id, checkpointId); await verifications.reload() }
    catch (error) { setMutationError(error instanceof Error ? error.message : 'Recovery Readiness Could Not Be Verified.') }
    finally { setVerificationBusyCheckpoint(null) }
  }
  ...
  return <div className="page-reveal">
    ...
    {workspace.loading && !workspace.data ? <LoadingState variant="table" /> : workspace.error ? <ErrorState message={workspace.error} retry={() => void workspace.reload()} /> : <section className="panel change-set-workspace" aria-label="Signed Change Set Workspace">
      ...
      {selected ? <ChangeSetDossier changeSet={selected} admin={user?.role === 'admin'} currentUserId={user?.id} validations={validations.data ?? []} checkpoints={checkpoints.data ?? []} verifications={verifications.data ?? []} validationLoading={validations.loading || checkpoints.loading || verifications.loading} validationError={validations.error || checkpoints.error || verifications.error || mutationError || null} validationBusyAgent={validationBusyAgent} checkpointBusyValidation={checkpointBusyValidation} verificationBusyCheckpoint={verificationBusyCheckpoint} authorize={() => void mutate(() => api.authorizeRemediationChangeSet(selected.id))} cancel={() => { setMutationError(''); setCanceling(true) }} queueValidation={agentId => void queueValidation(agentId)} queueCheckpoint={validationId => void queueCheckpoint(validationId)} queueVerification={checkpointId => void queueVerification(checkpointId)} /> : <aside className="change-set-dossier change-set-dossier-empty"><ShieldCheck size={24} /><h2>Select Or Prepare A Change Set</h2><p>Readiness gates, canary scope, cryptographic proof, and authorization history will appear here.</p></aside>}
    </section>}
