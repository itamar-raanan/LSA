import { Activity, ArrowRight, Gauge, ShieldAlert, ShieldCheck, ShieldX, WifiOff } from 'lucide-react'
...
export function DashboardPage() {
  const { data, error, loading, reload } = useApi(async () => {
    const [dashboard, hosts, findings] = await Promise.all([api.dashboard(), api.hosts(), api.findings()])
    return { dashboard, hosts, findings }
  }, [])
  ...
  const { dashboard, hosts, findings } = data
  ...
  const recentEvents = [...hosts]
    ...
    .map((host) => ({
      ...
      icon: Activity,
      to: `/hosts?host=${encodeURIComponent(host.id)}`,
    }))
  ...
  return <div className="page-reveal">
    ...
    <section className="mt-4 grid gap-4 xl:grid-cols-[1.2fr_0.8fr]">
      ...
      <article className="soc-panel overflow-hidden">
        ...
        <div className="dashboard-risk-list">{riskHosts.map((host, index) => <Link key={host.id} to={`/hosts?host=${encodeURIComponent(host.id)}`} className="dashboard-risk-host">
