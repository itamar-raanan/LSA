import { ArrowLeft, CaretDown, FileText, HardDrive, Package, ShieldCheck, ShieldWarning, SquaresFour } from '@phosphor-icons/react'
...
export function HostDetailPage() {
  ...
  const activeTab = requestedTab === 'findings' || requestedTab === 'applications' || requestedTab === 'evidence' ? requestedTab : 'overview'
  ...
  function selectTab(tab: 'overview' | 'findings' | 'applications' | 'evidence') {
  ...
  return (
    <div className="page-reveal">
      ...
      <TabList label="Host Record" className="mb-4 border-b border-stone-200">
        <TabButton active={activeTab === 'overview'} onClick={() => selectTab('overview')}><SquaresFour size={15} /> Overview</TabButton>
        <TabButton active={activeTab === 'findings'} onClick={() => selectTab('findings')}><ShieldWarning size={15} /> Findings <span className="tab-count">{host.finding_counts.critical + host.finding_counts.high + host.finding_counts.medium + host.finding_counts.low}</span></TabButton>
        <TabButton active={activeTab === 'applications'} onClick={() => selectTab('applications')}><Package size={15} /> Applications <span className="tab-count">{host.application_count ?? 0}</span></TabButton>
        <TabButton active={activeTab === 'evidence'} onClick={() => selectTab('evidence')}><FileText size={15} /> Evidence</TabButton>
      </TabList>
      {activeTab === 'overview' && <section className="grid gap-4 lg:grid-cols-[1.3fr_0.7fr]">
        ...
        <div className="panel p-6 md:p-8">
          ...
          <div className="mt-8 space-y-3 border-t border-stone-200 pt-6">
            ...
            <div className="flex items-center justify-between text-xs"><span className="flex items-center gap-2 text-stone-500"><ShieldWarning size={16} /> High findings</span><span className="font-mono text-orange-700">{host.finding_counts.high}</span></div>
      ...
      {activeTab === 'applications' && <ApplicationInventory hostId={host.id} />}
      {activeTab === 'evidence' && <ReportHistory hostId={host.id} />}
    </div>
  )
}
