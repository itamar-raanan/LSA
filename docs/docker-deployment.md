...
Managed agent release 0.9.0 keeps HTTPS encryption but does not validate the agent gateway certificate or hostname. Its console-generated install command pins a tenant-specific platform public key; enrollment and every subsequent platform control response are verified through short-lived, agent-bound, replay-protected Ed25519 envelopes. Replacement keys use a cross-signed, agent-acknowledged two-phase rotation before activation. Administrators can explicitly deliver a validation-only remediation contract over the agent listener. The agent performs read-only preflight and recovery-planning checks and returns an agent-signed no-change receipt; no backup writer, remediation task, or configuration-write executor exists. Management browsers and identity-provider callbacks continue to use normal TLS validation on port 8443.
