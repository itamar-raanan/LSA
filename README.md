...
| **Network** | Outbound TCP 8444 to the LSA agent gateway | Reliable DNS and time synchronization; agent 0.10.0 encrypts HTTPS traffic and authenticates platform control responses with a pinned Ed25519 identity, but does not validate the gateway certificate or hostname |
...
Managed agents advertise `signed-change-set-planning-v1` when they can provide the identity, policy, freshness, and integrity evidence required by this workflow. Agent 0.10.0 additionally supports validation, recovery planning, and explicitly requested encrypted checkpoints. After a successful preflight, an administrator can prepare AES-256-GCM backup material under `/var/lib/lsa-agent/remediation-checkpoints`; the root-only encryption key remains on that agent and backup contents never enter the platform receipt. A durable journal records each completed checkpoint, encrypted blob digest, and failure. The checkpoint store is capped at 256 MiB. Host configuration, restore, service control, sysctl changes, and remediation execution remain unavailable.
