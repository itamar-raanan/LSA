...
| **Network** | Outbound TCP 8444 to the LSA agent gateway | Reliable DNS and time synchronization; agent 0.9.0 encrypts HTTPS traffic and authenticates platform control responses with a pinned Ed25519 identity, but does not validate the gateway certificate or hostname |
...
Managed agents advertise `signed-change-set-planning-v1` when they can provide the identity, policy, freshness, and integrity evidence required by this workflow. Agent 0.9.0 additionally advertises validation, dry-run, and recovery-planning capabilities. An administrator can explicitly queue an authorized target for a separate, platform-signed dry-run delivery. The agent validates the complete contract, performs read-only local readiness checks, maps every required backup to an exact reverse-order rollback checkpoint, and returns an independently signed receipt with `changes_applied: false`. Symbolic links, ambiguous restores, oversized sources, and overlapping actions fail closed. This protocol does not create an `AgentTask` or grant configuration-write support.
...
Agent 0.6 adds two-phase platform signing-key rotation. The current and replacement keys cross-sign a proposal, every active compatible agent acknowledges the new fingerprint, and activation remains blocked until the complete fleet is ready. Activation revokes enrollment tokens tied to the retired identity; create a new token after rotation.
